...
class ObjectMaker {
	 ...
	 */
	static public function FromMovieClip(mc : MovieClip, addToCurrentState : Bool = true) : Array<Dynamic> {
		var a : Array<Dynamic> = [];
		var n : Float = mc.numChildren;
		var child : MovieClip;
		var i : Int = 0;
		while(i < n) {
			child = try cast(mc.getChildAt(i), MovieClip) catch(e) null;
			if(child)  {
				if(!child.className)  {
					i++;
					continue;
...
;
		...
		if(addToCurrentState)  {
			...
			for(object in a)ce.state.add(object);
		}
		return a;
	}
	 ...
	 * 
	 */
	static public function FromGleed(levelXML : XML, addToCurrentState : Bool = true, layerIndexProperty : String = "group", defaultClassName : String = "com.citrusengine.objects.CitrusSprite") : Array<Dynamic> {
		var layerIndex : UInt = 0;
		var items : Array<Dynamic> = [];
		var object : Dynamic;
		var objectName : String;
		var ce : CitrusEngine = CitrusEngine.getInstance();
		for(var layerXML : XML in levelXML.Layers.Layer)//Loop through all layers
		 {
			for(var itemXML : XML in layerXML.Items.Item)//Loop through all items on a layer
			 {
				//Grab the XML properties we want off of the item node.
				objectName = itemXML.@Name.toString();
				var x : Float = itemXML.Position.X.toString();
				//Top for primitives, center for textures
				var y : Float = itemXML.Position.Y.toString();
				...
				if(viewString != "")  {
					...
					var originX : Float = itemXML.Origin.X.toString();
					var originY : Float = itemXML.Origin.Y.toString();
					var rotation : Float = Std.parseFloat(itemXML.Rotation.toString()) /* WARNING check type */ * 180 / Math.PI;
					object = {
						x : x
						y : y,
						width : originX * 2,
					...
					var width : Float = itemXML.Width.toString();
					var height : Float = itemXML.Height.toString();
					object = {
						x : x + (width / 2)
						y : y + (height / 2),
						width : width,
				...
				if(layerIndexProperty) object[layerIndexProperty] = layerIndex;
				var className : String = defaultClassName;
				for(var customPropXML : XML in itemXML.CustomProperties.Property) {
					if(customPropXML.@Name.toString() == "className") className = customPropXML.string.toString()
					else object[customPropXML.@Name.toString()] = customPropXML.string.toString();
				}

				//Make the CitrusObject and add it to the current state.
				var citrusClass : Class<Dynamic> = Type.getClass(getDefinitionByName(className));
				var citrusObject : CitrusObject = new CitrusClass(objectName, object);
				if(addToCurrentState) ce.state.add(citrusObject);
			}
	 ...
	 */
	static public function FromLevelArchitect(levelData : XML, addToCurrentState : Bool = true) : Array<Dynamic> {
		var array : Array<Dynamic> = [];
		var state : State = try cast(CitrusEngine.getInstance().state, State) catch(e) null;
		for(var objectXML : XML in levelData.CitrusObject) {
			var params : Dynamic = { };
			for(var paramXML : XML in objectXML.Property) {
				params[paramXML.@name] = paramXML.toString();
			}

			var className : String = objectXML.@className;
			try {
				var theClass : Class<Dynamic> = Type.getClass(getDefinitionByName(className));
			}
			catch(e : Error) {
				if(e.errorID == 1065)  {
			...
			if(addToCurrentState) state.add(theObject);
		}

		return array;
	}

	static function Replace(str : String, fnd : String, rpl : String) : String {
