...
class BlittingArt {
	 ...
	 */
	public var addIndex : Float;
	/**
	 * The CitrusObject associated with this art object.
	 */
	public var citrusObject : Dynamic;
	/**
	 ...
	 * backgrounds.
	 */
	public function new(defaultGraphic : Class<Dynamic> = null) {
		registration = "center";
		offset = new MathVector();
		sequences = [];
		group = 0;
		addIndex = 0;
		if(defaultGraphic) addAnimation(new AnimationSequence(defaultGraphic));
	}

	public function initialize(citrusObject : Dynamic) : Void {
	...
	public function play(animationName : String) : Void {
		...
		if(!newAnimation || newAnimation.name == currAnimation.name) return;
	...
	public function addAnimation(animationSequence : AnimationSequence) : Void {
		...
		if(!currAnimation) currAnimation = animationSequence;
	}
	...
	public function getAnimationByName(name : String) : AnimationSequence {
		for(var sequence : AnimationSequence in sequences) {
