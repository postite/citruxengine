...
class Console extends Sprite {
	...
	public function new(openKey : Int = 9) {
		...
		if(_shared.data.history)  {
			_commandHistory = try cast(_shared.data.history, Array) catch(e) null;
		...
		_inputField = try cast(addChild(new TextField()), TextField) catch(e) null;
...
;
					 {
						trace(e.message);
						var expected : Float = Std.parseFloat(e.message.slice(e.message.indexOf("Expected ") + 9, e.message.lastIndexOf(","))) /* WARNING check type */;
