...
class Missile extends PhysicsObject {
	...
	var _velocity : V2;
	var _exploded : Bool;
	var _explodeTimeoutID : Float;
	var _fuseDurationTimeoutID : Float;
	var _contact : PhysicsObject;
	static public function Make(name : String, x : Float, y : Float, width : Float, height : Float, angle : Float, view : Dynamic = null, speed : Float = 2, fuseDuration : Float = 10000, explodeDuration : Float = 1000) : Missile {
		if(view == null) view = MovieClip;
		return new Missile(name, {
			x : x
			y : y,
			width : width,
			height : height,
	...
	public function explode() : Void {
		...
		_fixture.SetFilterData({
			maskBits : CollisionCategories.GetNone()
	...
	function handleBeginContact(e : ContactEvent) : Void {
		_contact = try cast(e.other.GetBody().GetUserData(), PhysicsObject) catch(e) null;
		if(!e.other.IsSensor()) explode();
	}
